...
function PersonProfile() {
	...
	// Fetch sources for citation form dropdown
	useEffect(() => {
		fetch('/api/sources/')
			.then(res => res.json())
	...
	const handleDeleteCitation = async (citationId, eventId) => {
		if (window.confirm('Delete this citation?')) {
			await fetch(`/api/citations/${citationId}/`, { method: 'DELETE' });
			// Update the events state to remove the citation
			setEvents(events.map(event => 
				event.id === eventId 
					? { ...event, citations: event.citations.filter(c => c.id !== citationId) }
					: event
			));
		}
	};
				...
				<ul>
					{events.map(event => (
						<li key={event.id}>
							...
							{/* Citation indicator - clickable if citations exist, changes the arrow direction  when clicked */}
							{event.citations && event.citations.length > 0 && (
								<button 
									onClick={() => toggleCitations(event.id)}
									className="citation-toggle"
								>
									{event.citations.length} citation(s) {expandedEvents[event.id] ? '▼' : '▶'}
								</button>
							)}
							
							{/* Expanded citations */}
							{expandedEvents[event.id] && event.citations && (
								<div className="citations-expanded">
									{event.citations.map(citation => (
										<div key={citation.id} className="citation-detail">
											<p><strong>Source:</strong> {sources.find(s => s.id === citation.source)?.title || citation.source}</p>
											{citation.page && <p><strong>Page:</strong> {citation.page}</p>}
											{citation.line_number && <p><strong>Line:</strong> {citation.line_number}</p>}
											{citation.name_as_recorded && <p><strong>Name as Recorded:</strong> {citation.name_as_recorded}</p>}
											{citation.transcription && <p><strong>Transcription:</strong> {citation.transcription}</p>}
											{citation.notes && <p><strong>Notes:</strong> {citation.notes}</p>}
											<button className="delete-btn citation-delete" onClick={() => handleDeleteCitation(citation.id, event.id)}>
												Delete Citation
											</button>
										</div>
							...
							<div className="button-row">
								<button 
									onClick={() => navigate(`/citations/new?person=${id}&event=${event.id}`)}
									className="cite-btn">
									Cite
								</button>
...
function PeopleList() {
	...
	const filteredPeople = people.filter(person => 
        `${person.given_name} ${person.last_name}`.toLowerCase().includes(searchTerm.toLowerCase())
    );
